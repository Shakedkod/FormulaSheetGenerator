...
\usepackage{adjustbox}   % fit math in tables% ---------- Images & Diagrams ----------
...
\title{אינפי 2מ}
\author{שקד}
\date{ספטמבר 18, 2026 (ז׳ בתשרי ה'תשפ״ז)}
...
\end{example}
\begin{remark}
לא לכל \({f}\) יש פונקציה קדומה. לדוגמה:\begin{LTR}
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=6.00cm, height=6.00cm, xmin=-5, xmax=5, ymin=-1, ymax=2, axis lines=middle, samples=200, thick, xlabel={\(x\)}, ylabel={\(y\)}]
...
הפונקציה הקדומה אינה יכולה להיות הפונקציה הבאה שכן היא(הפונקציה הבאה) לא גזירה ב-\({0}\).\begin{LTR}
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=6.00cm, height=6.00cm, xmin=-5, xmax=5, ymin=-1, ymax=10, axis lines=middle, samples=200, thick, xlabel={\(x\)}, ylabel={\(y\)}]
