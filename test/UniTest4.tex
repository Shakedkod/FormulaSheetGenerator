...
\usepackage{adjustbox}   % fit math in tables% ---------- Misc quality-of-life ----------
...
\begin{itemize}
\item מוסיפה את \({x2}\) ו-\({x3}\) ושמה את הערך ב-\({x1}\).
\item מחסרת את \({x5}\) מ-\({x4}\) ומציבה את הערך ב-\({x3}\).
\end{itemize}
\begin{example}
...
\end{remark}
\begin{itemize}
\item הפקודה הזו מחברת את הערך ב-\({x4}\) לקבוע \({-10}\) ושמה את הערך ב-\({x3}\).\newline{}
...
\item מה זה \({12(x13)}\) המוזר הזה? \({x13}\) מייצג את הרגיסטר הבסיסי שמצביע ל-\({A[0]}\) ו-\({12}\) זה מספר הבתים שאנחנו זזים כדי לקחת את \({A[3]}\).\newline{}
אזי, בעצם אנחנו לוקחים את הערך הנמצא ב-\({A[3]}\) ושמים אותו ברגיסטר \({x10}\).
\item מכיוון שכל איבר במערך הוא 4 בתים, נכפיל 4 ב-10 כדי להגיע מ-\({x13}\) שמאחסן פויינטר לתחילת המערך, למיקום במערך שאנחנו רוצים.
\item כשמעתיקים בית מהזיכרון אל 32 בתים של רגיסטר, מתבצע תהליך של הרחבת הסימן על מנת שהסימן ישאר זהה, נניח אם הסימן הוא \({0}\) למספר חיובי, כל שאר הסיביות של הרגיסטר יהיו אפסים ואז בסוף יופיע המספר.
